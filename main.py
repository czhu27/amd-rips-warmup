...
class args:
	exp_id = "no_exp_id"
exp_folder = "results/" + args.exp_id + "/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
...
opt_num_its = 1000		# number of iterations
...
model.fit(dataset, 
		  epochs=opt_num_its, 
		  verbose=2,
		  callbacks=[TimeLogger(), StressTestLogger(), tensorboard_callback])
toc = time.time()
